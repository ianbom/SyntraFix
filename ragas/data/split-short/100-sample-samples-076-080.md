...
The software engineering industry is undergoing a rapid transformation driven by the emergence of large language models (LLMs) and their use to create autonomous agents that can perform complex software engineering tasks. This paradigm shift, which we call agentic software engineering (ASE), represents a significant rethinking of how software is designed, developed, and maintained [1], [2], [3].
...
FIGURE 8. Confusion matrix result of ball detection using CNN.
...
• Proposing a system for predicting the position of the ball using the ZED camera because the ZED camera has the ability to detect ball objects with extreme lighting conditions on the Goalkeeper Robot Application. • Proposing a system for predicting the ball's arrival position relative to the robot keeper using CNN.
...
1. Conv1D Layer • Function: Filtering the input signal using a filter. • Operation: where y is the input, w is the filter weight, b is the bias, and * is the convolution operation. • Output: Filtered signal.
